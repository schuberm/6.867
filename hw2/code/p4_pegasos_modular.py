	...
	for ni,name in enumerate(Name):
		for gj,gamma in enumerate(Gamma):
			for lk,lmbda in enumerate(Lmbda):
				...
				#pl.show()
				#pl.savefig('./figures/ps3.3_training'+str(name)+'_'+str(gamma)+'_'+str(lmbda)+'.pdf')
				#pl.close()
				print '======Validation======'
				if (ni < 3):
					vn = 150
				...
				corr = 0.0
				for i in range(n):
					corr = corr + np.abs(Yv[i]-np.sign(predict_gaussianSVM(Xv[i,:])))
				print(corr/2/Xv.shape[0])
				EM[ni,gj,lk,1] = corr/2/X.shape[0]

	#print(EM)
	np.save('EM_gaussian_p4.txt',EM)
	np.save('margin_gaussian_p4.txt',margin)
	np.save('NSV_gaussian_p4.txt',NSV)
