...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    h1 = np.maximum(0,W1.T.dot(np.reshape(X,(N,D)).T).T + b1)
    scores = (W2.T.dot(h1.T).T + b2)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    #print probs
    loss = np.sum(-np.log(np.exp(scores[range(len(y)), y])/np.sum(np.exp(scores),axis = 1)))/N
    #loss = np.sum(- scores[range(len(y)), y] + np.log( np.sum( np.exp(scores), axis=1) )) / N
    ...
    # finally into W,b
    grads['W1'] = np.dot(np.reshape(X,(N,D)).T, dhidden)
